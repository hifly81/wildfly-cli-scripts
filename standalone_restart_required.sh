#!/bin/bash
#title           :standalone_restart_required.sh
#description     :This script will check if the modification of an EAP resource property will require a restart.
#author		 :
#date            :20180402
#version         :0.1
#usage		 :bash standalone_restart_required.sh <subsystem> <controller_host> <controller_host_port>
...
then
  ...
  read property
  rel=$($CLI_HOME./jboss-cli.sh --connect --controller=$JBOSS_CONTROLLER:$JBOSS_CONTR_PORT -c "/subsystem=datasources/data-source="${resource_sample_name}":read-resource-description")
...
then
  ...
  read property
  rel=$($CLI_HOME./jboss-cli.sh --connect --controller=$JBOSS_CONTROLLER:$JBOSS_CONTR_PORT -c "/subsystem=datasources/xa-data-source="${resource_sample_name}":read-resource-description")
  print_resource "$rel" $property
fi
...
then
  ...
  read property
  rel=$($CLI_HOME./jboss-cli.sh --connect --controller=$JBOSS_CONTROLLER:$JBOSS_CONTR_PORT  -c "/subsystem=resource-adapters/resource-adapter="${resource_sample_name}"/admin-objects="${resource_sample_name}":read-resource-description")
  print_resource "$rel" $property
fi
...
then
  ...
  read property
  rel=$($CLI_HOME./jboss-cli.sh --connect --controller=$JBOSS_CONTROLLER:$JBOSS_CONTR_PORT  -c "/subsystem=resource-adapters/resource-adapter="${resource_sample_name}"/connection-definitions="${resource_sample_name}":read-resource-description")
  print_resource "$rel" $property
fi
...
then
  ...
  read property
  rel=$($CLI_HOME./jboss-cli.sh --connect --controller=$JBOSS_CONTROLLER:$JBOSS_CONTR_PORT  -c "/subsystem=resource-adapters/resource-adapter="${resource_sample_name}"/config-properties="${resource_sample_name}":read-resource-description")
  print_resource "$rel" $property
fi
...
then
  ...
  read property
  rel=$($CLI_HOME./jboss-cli.sh --connect --controller=$JBOSS_CONTROLLER:$JBOSS_CONTR_PORT  -c "/subsystem=messaging-activemq/jms-bridge="${resource_sample_name}":read-resource-description")
  print_resource "$rel" $property
fi
...
then
  ...
  read property
  rel=$($CLI_HOME./jboss-cli.sh --connect --controller=$JBOSS_CONTROLLER:$JBOSS_CONTR_PORT  -c "/subsystem=messaging-activemq/server="${resource_sample_name}"/connection-factory="${resource_sample_name}":read-resource-description")
  print_resource "$rel" $property
fi
...
then
  echo "Property to check: (valid are: attributes)"
  read property
  rel=$($CLI_HOME./jboss-cli.sh --connect --controller=$JBOSS_CONTROLLER:$JBOSS_CONTR_PORT  -c "/system-property="${resource_sample_name}":read-resource-description")
  print_resource "$rel" $property
fi
...
then
  ...
  read property
  rel=$($CLI_HOME./jboss-cli.sh --connect --controller=$JBOSS_CONTROLLER:$JBOSS_CONTR_PORT -c "/subsystem=mail/mail-session="${resource_sample_name}":read-resource-description")
  print_resource "$rel" $property
fi
...
then
  ...
  read property
  rel=$($CLI_HOME./jboss-cli.sh --connect --controller=$JBOSS_CONTROLLER:$JBOSS_CONTR_PORT -c "/subsystem=logging/periodic-rotating-file-handler="${resource_sample_name}":read-resource-description")
  print_resource "$rel" $property
fi
...
then
  ...
  read property
  rel=$($CLI_HOME./jboss-cli.sh --connect --controller=$JBOSS_CONTROLLER:$JBOSS_CONTR_PORT -c "/subsystem=logging/periodic-size-rotating-file-handler="${resource_sample_name}":read-resource-description")
  print_resource "$rel" $property
fi
...
then
  ...
  read property
  rel=$($CLI_HOME./jboss-cli.sh --connect --controller=$JBOSS_CONTROLLER:$JBOSS_CONTR_PORT -c "/subsystem=logging/size-rotating-file-handler="${resource_sample_name}":read-resource-description")
  print_resource "$rel" $property
fi
...
then
  ...
  read property
  rel=$($CLI_HOME./jboss-cli.sh --connect --controller=$JBOSS_CONTROLLER:$JBOSS_CONTR_PORT -c "/subsystem=logging/logger="${resource_sample_name}":read-resource-description")
  print_resource "$rel" $property
fi
